    ...
    @can('add-user')
      <div class="col-lg-12 col-md-12 col-sm-12 mb-20">
        <div class="pd-20 card-box height-100-p">
          <a href="javascript:void(0)" class="btn btn-info btn-md float-right mb-3" data-toggle="modal" data-target="#userModal"><i class="icon-copy dw dw-add-user"></i> add new</a>
          <!-- Modal -->
          ...
          <tbody>
            @foreach ($users as $item)
              <tr>
                ...
                <td>
                  @can('edit-user')
                    <a href="{{ route('user-profile.edit', $item->id) }}" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="top" title="user-edit"><i
                        class="icon-copy dw dw-edit2"></i></a>
                  @endcan
                  @can('delete-user')
                    <a href="{{ route('user-profile.destroy', $item->id) }}" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="top" title="user-delete"
                      onclick="return confirm('Are you sure to remove this user?')"><i class="icon-copy dw dw-delete-3"></i></a>
                  @endcan
                  @can('add-userPermission')
                    <a href="{{ route('permission.create', $item->id) }}" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="top" title="manage-permissions"><i
                        class="icon-copy dw dw-settings1"></i></a>
                  @endcan
                </td>
              </tr>
            @endforeach
